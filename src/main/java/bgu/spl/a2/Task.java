...
public abstract class Task<R> {
	private Deferred<R> myDeffered;
	private Processor myHandler;
	private int NumOfTaskToBeDone;
    /**
     ...
     */
    protected final void spawn(Task<?>... task) {///dont forget the dependency list
    	for(int i=0;i<task.length;i++){
    		myHandler.getMyQueue().addFirst(task[i]);
    	}
    }
	...
	protected final void whenResolved(Collection<? extends Task<?>> tasks, Runnable callback) {
		setNumOfTaskToBeDone(tasks.size());
		Object[] arr = tasks.toArray();
		for (int i = 0; i < arr.length; i++) {
			((Task<?>) arr[i]).getMyDeferred().whenResolved(() -> {
				decreaseNumOfTaskToBeDone();
				if (getNumOfTaskToBeDone() == 0)
					callback.run();
			});
    ...
    void setNumOfTaskToBeDone(int x){
    	NumOfTaskToBeDone=x;
    }
    void decreaseNumOfTaskToBeDone(){
    	NumOfTaskToBeDone=NumOfTaskToBeDone-1;
    }
    int getNumOfTaskToBeDone(){
    	return NumOfTaskToBeDone;
    }
    /**
